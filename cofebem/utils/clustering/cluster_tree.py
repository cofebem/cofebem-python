"""
Cluster nodes using PCA clustering
"""
    ...
    def __init__(self, cl_id, ids, coords, parent=None):
        ...
        if self.dim == 3:
            ...
            self.center = np.array(
                [
                    np.mean(self.coords[:, 0]),
                    np.mean(self.coords[:, 1]),
                    np.mean(self.coords[:, 2]),
                ]
            )
            assert self.center[2] >= self.minz and self.center[2] <= self.maxz
        elif self.dim == 2:
            self.size = np.sqrt(
                (self.maxx - self.minx) ** 2 + (self.maxy - self.miny) ** 2
            )
            self.center = np.array(
                [np.mean(self.coords[:, 0]), np.mean(self.coords[:, 1])]
            )
...
class ClusterTree:
    ...
    def recursive_pca_clustering(self, cluster, min_cluster_size, level):
        if cluster.length < min_cluster_size:
            ...
            if (
                len(cluster1_ids) >= min_cluster_size
                and len(cluster2_ids) >= min_cluster_size
            ):
        ...
        if cl1 is not cl2:
            ...
            if max_size < 1.0e-10 * dist:
                ...
                if cl1.children is None or cl2.children is None:
                    self.DirectComputeBlock(
                        cl1, cl2
                    )  # Directly compute as no further subdivision possible
                else:
    ...
    def plot_hierarchical_matrix(self, filename, matrix, palette, value_min, value_max):
        ...
        matrix_visual = np.full(
            (num_clusters, num_clusters), -1
        )  # Initialize with -1 for no interaction
        ...
        for ci in range(num_clusters):
            ...
            ax.imshow(
                np.log(matrix[np.ix_(coefficients, coefficients)]),
                vmin=value_min,
                vmax=value_max,
                cmap=palette,
                alpha=1,
                extent=(ci - 0.5, ci + 0.5, ci - 0.5, ci + 0.5),
                zorder=1,
            )
        for interaction in self.hierarchical_matrix:
            ...
            level = (
                len(cl1.cl_id) - 1
            )  # Assume level is deduced from the length of cl_id
            ...
            ax.imshow(
                np.log(matrix[np.ix_(coefficients_x, coefficients_y)]),
                cmap=palette,
                alpha=1,
                extent=(
                    idx1 - 0.5,
            ...
            ax.imshow(
                np.log(matrix[np.ix_(coefficients_y, coefficients_x)]),
                cmap=palette,
                alpha=1,
                extent=(
                    idx2 - 0.5,
        ...
        cax = ax.matshow(
            matrix_visual, cmap=palette, vmin=0, vmax=2, alpha=0.0
        )  # Set vmin and vmax for color consistency
    ...
    def plot_hierarchical_matrix_structure(self, filename):
        ...
        cax = ax.matshow(
            matrix_visual, cmap="viridis", vmin=0, vmax=2, alpha=0.0
        )  # Set vmin and vmax for color consistency
