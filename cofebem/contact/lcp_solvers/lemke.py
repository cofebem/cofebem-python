...
class lemketableau:
    def __init__(self, S_c, g0, maxIter=100):
        n = len(g0)
        self.T = np.hstack((np.eye(n), -S_c, -np.ones((n, 1)), g0.reshape((n, 1))))
        self.n = n
        ...
        self.Y = 2
        self.Q = 3
        TbInd = np.vstack((self.W * np.ones(n, dtype=int), np.arange(n, dtype=int)))
        TnbInd = np.vstack((self.Z * np.ones(n, dtype=int), np.arange(n, dtype=int)))
    ...
    def indexStringArray(self):
        indstr = np.array(
            [self.ind2str(indvec) for indvec in self.Tind.T], dtype=object
        )
        return indstr
