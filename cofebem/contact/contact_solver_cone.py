...
def constrained_CG(K, error_type, coord, dofs, gap, max_iter, tolerance, pressure_factor=100, initial_pressure=None):
    ...
    for iter in range(max_iter):
        ...
        if error_type == "displacement":
            ...
            if abs((error - error_)/error_) < tolerance:
                ...
                break
        # error = np.sqrt(displ_error * ort) # The most efficient according to my tests
...
# displ = 0.15 # for conical indenter
displ = 0.15
Rindenter = .31
...
ANIMATION = True
if ANIMATION == True:
    x_center = np.linspace(-0.3,1.3,21)
    ...
    for frame, xc in enumerate(x_center):
        ...
        if frame == 0:
            pressure, displacement, error_history = constrained_CG(K, error_type, coords, dofs, gap, max_iter, tolerance, pfactor) #.0001
        else:
