...
def _boussinesq_surface_kernel(rvec, E, nu):
    r = np.linalg.norm(rvec)
    if r <= 0.0:
        raise ValueError("Boussinesq kernel undefined at r=0 without regularization.")
    return (1.0 - nu * nu) / (np.pi * E * r)
...
def _source_integral_regular(
    ...
    val = 0.0
    ndot = float(np.dot(n_target, n_source))
    ...
    for c, us in enumerate(xq_s):
        ...
        for d, vs in enumerate(xq_s):
            ...
            U = _boussinesq_surface_kernel(rvec, E, nu)
            val += ndot * U * phi_s * Js * Jrs * wus * wvs
...
def _source_integral_telles(
    ...
    for a, uhat in enumerate(xq):
        ...
        for b, vhat in enumerate(xq):
            ...
            U = _boussinesq_surface_kernel(rvec, E, nu)
            contrib = ndot * U * phi_s * Js * Jtri * Ju * Jv * wu * wv
...
def _source_integral_self_duffy(
    ...
    for c, us in enumerate(xq_s):
        ...
        for d, vs in enumerate(xq_s):
            ...
            U = _boussinesq_surface_kernel(rvec, E, nu)
            val += ndot * U * phi_s * Js * Jds * wus * wvs

    return val


def Sc_Boussinesq_source_integrated(
    mesh,
    facet_ids,
    contact_vertex_ids,
    xc,
    ...
    it = range(nc)
    if show:
        it = tqdm(it, desc="Building source integrated Boussinesq Snn", unit="col")
    ...
    for j in it:
        ...
        if Aj <= 0.0:
            raise RuntimeError(f"Zero nodal area detected at contact node {j}.")
...
Sc_Boussinesq_source = Sc_Boussinesq_source_integrated(
    mesh=cube,
...
R_source = Sc_FE - Sc_Boussinesq_source

_, s_FE, _ = np.linalg.svd(Sc_FE)
_, s_Boussinesq_source, _ = np.linalg.svd(Sc_Boussinesq_source)
...
ax.semilogy(
    np.arange(1, len(s_Boussinesq_source) + 1),
    s_Boussinesq_source,
    marker="s",
    markersize=4,
    linewidth=1.8,
    label=r"$S_c^{Boussinesq,src}$",
)
ax.semilogy(
    ...
    markersize=4,
    linewidth=1.8,
    label=r"$S_c^{FE}-S_c^{Boussinesq,src}$",
)
ax.set_title("Source integrated Boussinesq")
...
print("||Sc_Boussinesq_source||F     =", np.linalg.norm(Sc_Boussinesq_source, "fro"))
...
print(
    "Boussinesq source integrated  :",
    np.min(np.diag(Sc_Boussinesq_source)),
    np.max(np.diag(Sc_Boussinesq_source)),
)
