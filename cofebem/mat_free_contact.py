...
if __name__ == "__main__":
    ...
    q_gamma = np.asarray(g0, dtype=np.float64).reshape(-1)

    assert Ic_gamma.size == q_gamma.size
    ...
    # Solve the full LCP on Gamma_c
    p, gap, info = gpcg_matrix_free(
        apply_M=apply_Sc,
        q=q_gamma,
        z0=p0,
        tol=1e-10,
    ...
    with VTKFile(mesh.comm, f"./results/matfree/cube.pvd", "w") as vtk:
        vtk.write_mesh(mesh)
        vtk.write_function([u_fenics, p_fenics], 0)

    # Visualization
    solver_petsc = PETSc.KSP().create(mesh.comm)
    solver_petsc.setOperators(pb.A)
