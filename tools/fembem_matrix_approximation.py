"""
Construct hierarchical matrix from a given matrix using adaptive cross approximation (ACA).
...
FileName = f"{Test}_eta_{Eta}.pdf"
if Test == "Dragon":
    dae_name = "../data/dragon_model.dae"
    ...
    # Create a dummy matrix for visualization (you may want to replace this with your actual matrix)
    n_vertices = len(mesh.vertices)
    dummy_matrix = np.random.rand(n_vertices, n_vertices)  # Replace with your actual matrix
    ctree.plot_hierarchical_matrix(FileName, dummy_matrix, palette="inferno") 
    ctree.plot_hierarchical_matrix_structure("Test1.pdf")
    ctree.plot_coarse_hierarchical_matrix_structure("Test2.pdf", min_size = min_cluster_size)
